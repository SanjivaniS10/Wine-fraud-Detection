...
st.title("🍷 Wine Fraud Detection — Streamlit App")
st.markdown(
    """
Upload your `wine_fraud.csv` or use a sample. The app trains a (balanced) SVM, shows EDA,
and lets you predict single samples.
"""
)
...
st.markdown(
    """
- This app follows your notebook flow: dummy-encoding `type` (drop_first=True), scaling, and SVM with `class_weight='balanced'`.
- Because class imbalance is strong, consider SMOTE or other resampling, or trying tree-based models (RandomForest / XGBoost) with class weights.
- If you upload a pretrained `svm_model.pkl`, include both `scaler` and `estimator` (as a dict) to ensure correct predictions.
"""
)
